...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --output_root|--output-root)
      ...
      shift 2
      ;;
    --youtube_vos_root|--youtube-vos-root)
      YOUTUBE_VOS_ROOT="$2"
      shift 2
      ;;
    --caption_json|--caption-json)
      CAPTION_JSON="$2"
      shift 2
      ;;
...
out_root="${OUTPUT_ROOT:-$repo_root/data/generated_losers/official_videodpo_diffueraser_youtubevos_partialmask_loser_k4}"
shards_root="${SHARDS_ROOT:-$out_root/_shards}"
report_path="${REPORT_PATH:-$out_root/reports/generated_loser_youtubevos_generation_report.md}"
...
if [ "$models_csv" != "diffueraser" ]; then
  echo "[error] D3省时版只允许 MODELS=diffueraser；不要在当前主线恢复 all-model generation。" >&2
  exit 2
fi
...
if [ ! -d "$youtube_vos_root" ]; then
  echo "[error] YOUTUBE_VOS_ROOT not found: $youtube_vos_root" >&2
  exit 2
fi
if [ -z "${DIFFUERASER_PYTHON:-}" ] || [ ! -x "$DIFFUERASER_PYTHON" ]; then
  echo "[error] DIFFUERASER_PYTHON not found/executable" >&2
  exit 2
fi
...
if [ -z "$end_index" ]; then
  end_index="$("$orchestration_python" tools/youtubevos_generated_loser_calibration.py \
...
echo "===== H20 YOUTUBE-VOS D3 PARTIALMASK K4 DIFFUERASER GENERATION ====="
...
run_shard() {
  ...
  (
    ...
    exec -a "$LINGBOT_PROCESS_NAME" "$orchestration_python" tools/youtubevos_generated_loser_calibration.py \
