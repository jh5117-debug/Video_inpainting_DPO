...
class Exp23RunnerPassesOuterTest(unittest.TestCase):
    def test_runner_fresh_control_passes_outer_boundary_mode(self):
        region = RegionConfig(True, "outer", 1, 0, 1, 0.0, 0.75, 0.75)
        args = base_args(Path("/tmp/fresh"), RunConfig(pair_id="unit"), region, stage=1)
    ...
    def test_runner_candidate_passes_outer_boundary_mode_and_outer2(self):
        region = RegionConfig(False, "outer", 1, 0, 2, 0.0, 0.75, 0.75)
        args = base_args(Path("/tmp/candidate"), RunConfig(pair_id="unit"), region, stage=1)
        self.assertEqual(_option_value(args, "--boundary_mode"), "outer")
