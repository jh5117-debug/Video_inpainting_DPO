#!/usr/bin/env python3
"""Evaluate locked Exp26 VideoPainter search-dev step0 outputs.
...
def build_pair_rows(mask_manifest: Path, run_root: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[str]]:
    ...
    for row in rows:
        ...
        raw_pairs.append({**base, "model_label": "VideoPainter_step0_raw", "prediction_video_path": str(raw)})
        comp_pairs.append({**base, "model_label": "VideoPainter_step0_comp", "prediction_video_path": str(comp)})
...
def main() -> int:
    ...
    review_dir = args.review_dir or (run_root / "step0_review")
    mask_manifest = args.mask_manifest or (run_root / "gate64_mask_ready.jsonl")
    eval_dir = run_root / "step0_metric_eval"
    eval_dir.mkdir(parents=True, exist_ok=True)

    raw_pairs, comp_pairs, issues = build_pair_rows(mask_manifest, run_root)
    raw_manifest = eval_dir / "step0_raw_metric_pairs.jsonl"
    comp_manifest = eval_dir / "step0_comp_metric_pairs.jsonl"
    ...
    write_csv(report_dir / "exp26_vp_step0_baseline.csv", final_rows)
    if review_csv_path.exists():
        (report_dir / "exp26_vp_step0_visual_review.csv").write_text(review_csv_path.read_text(encoding="utf-8"), encoding="utf-8")
    ...
    (report_dir / "exp26_vp_step0_baseline.md").write_text("\n".join(md) + "\n", encoding="utf-8")
