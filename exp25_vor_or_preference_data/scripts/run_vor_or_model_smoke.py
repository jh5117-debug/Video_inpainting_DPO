#!/usr/bin/env python3
"""Run small VOR OR loser-generation smoke for verified model wrappers."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--model", choices=["diffueraser", "propainter", "effecterase"], required=True)
...
def main() -> int:
    ...
    results = []
    if args.model == "effecterase":
        status = {
            "model": "effecterase",
            "status": "BLOCKED_WRAPPER_NOT_RESOLVED",
            "reason": "No verified EffectErase inference wrapper/checkpoint path exists in Exp25 worktree; refusing fallback.",
        }
        (args.output_root / "effecterase_blocked.json").write_text(json.dumps(status, indent=2) + "\n", encoding="utf-8")
