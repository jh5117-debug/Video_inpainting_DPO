...
if [[ "${1:-}" == "--help" ]]; then
  ...
  echo "Plans partial-mask + comp offline loser generation. Default is dry run."
  exit 0
fi

MODEL_NAME="${MODEL_NAME:-diffueraser}"
SOURCE_DATASET="${SOURCE_DATASET:-${VIDEO_DPO_DATA_ROOT:-}}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${GENERATED_LOSER_ROOT:-$REPO_ROOT/data/generated_losers/partialmask_comp}}"
