...
from exp18_dataset import Exp18PropagationManifestDataset  # noqa: E402
from exp18_loss import (  # noqa: E402
    PropagationConfidenceConfig,
    compute_propagation_gated_losses,
    predict_x0_from_model_output,
    summarize_propagation_confidence,
)
...
def append_dpo_diagnostics_csv(output_dir, step, diag, grad_norm=None):
    ...
    row = {
        ...
        "L_prop": diag.get("L_prop"),
        "L_gen": diag.get("L_gen"),
        "L_boundary": diag.get("L_boundary"),
        "lambda_prop": diag.get("lambda_prop"),
        "lambda_gen": diag.get("lambda_gen"),
        "lambda_boundary_extra": diag.get("lambda_boundary_extra"),
        "exp18_extra_loss": diag.get("exp18_extra_loss"),
        ...
        "propagation_coverage": diag.get("propagation_coverage"),
        "generate_area_ratio": diag.get("generate_area_ratio"),
        "prop_conf_mean": diag.get("prop_conf_mean"),
        "prop_conf_p10": diag.get("prop_conf_p10"),
        "prop_conf_p50": diag.get("prop_conf_p50"),
        "prop_conf_p90": diag.get("prop_conf_p90"),
        "avg_num_sources_used": diag.get("avg_num_sources_used"),
        "propagated_region_psnr": diag.get("propagated_region_psnr"),
        "tau_conf": diag.get("tau_conf"),
        "confidence_mode": diag.get("confidence_mode"),
        "propagation_target_mode": diag.get("propagation_target_mode"),
    }
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--exp18_require_propagation", type=parse_bool_arg, default=True,
                        help="Require real multi-frame propagation frames and confidence maps.")
    parser.add_argument("--confidence_mode", type=str, default="flow_agreement", choices=["flow_agreement", "oracle"])
    parser.add_argument("--tau_conf", type=float, default=0.5)
    parser.add_argument("--lambda_prop", type=float, default=0.1)
    ...
    parser.add_argument("--preflight_only", action="store_true",
                        help="Run one Exp18 batch forward/backward and write diagnostics, then exit before optimizer.step.")
...
def collate_fn(examples):
    ...
    if "propagated_pixel_values" in examples[0]:
        batch["propagated_pixel_values"] = torch.stack([e["propagated_pixel_values"] for e in examples]).float()
        batch["propagation_confidence"] = torch.stack([e["propagation_confidence"] for e in examples]).float()
        batch["propagated_frame_dir"] = [e.get("propagated_frame_dir") for e in examples]
        batch["propagation_confidence_dir"] = [e.get("propagation_confidence_dir") for e in examples]
        if all(e.get("avg_num_sources_used") is not None for e in examples):
            batch["avg_num_sources_used"] = torch.tensor([float(e.get("avg_num_sources_used")) for e in examples])
        if all(e.get("propagated_region_psnr") is not None for e in examples):
            batch["propagated_region_psnr"] = torch.tensor([float(e.get("propagated_region_psnr")) for e in examples])
    return batch
...
def main(args):
    ...
    if args.exp18_require_propagation:
        train_dataset = Exp18PropagationManifestDataset(args, tokenizer)
        logger.info("Using Exp18PropagationManifestDataset with real multi-frame propagation cache paths.")
    else:
        raise ValueError("Exp18 training requires --exp18_require_propagation true; proxy/no-propagation mode is not allowed.")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet_main, brushnet):
                ...
                if "propagated_pixel_values" not in batch or "propagation_confidence" not in batch:
                    raise RuntimeError(
                        "Exp18 requires propagated_pixel_values and propagation_confidence from a real "
                        "multi-frame propagation manifest."
                    )
                debug_stage("before exp18 propagation confidence stats")
                prop_confidence = batch["propagation_confidence"].to(dtype=torch.float32)
                avg_num_sources_used = None
                propagated_region_psnr = None
                if isinstance(batch.get("avg_num_sources_used"), torch.Tensor):
                    avg_num_sources_used = float(batch["avg_num_sources_used"].float().mean().item())
                if isinstance(batch.get("propagated_region_psnr"), torch.Tensor):
                    propagated_region_psnr = float(batch["propagated_region_psnr"].float().mean().item())
                prop_conf_stats = summarize_propagation_confidence(
                    prop_confidence,
                    (1.0 - batch["masks"].float()).clamp(0.0, 1.0),
                    tau_conf=args.tau_conf,
                    avg_num_sources_used=avg_num_sources_used,
                    propagated_region_psnr=propagated_region_psnr,
                )
                debug_stage("after exp18 propagation confidence stats")

                debug_stage("before vae propagation encode")
                prop_latents = vae.encode(
                    rearrange(batch["propagated_pixel_values"], "b f c h w -> (b f) c h w").to(dtype=vae_dtype)
                ).latent_dist.sample() * vae.config.scaling_factor
                prop_latents = prop_latents.to(dtype=policy_dtype)
                debug_stage("after vae propagation encode")
                ...
                del batch["pixel_values_pos"], batch["pixel_values_neg"], batch["conditioning_pixel_values"], batch["propagated_pixel_values"]
                ...
                exp18_cfg = PropagationConfidenceConfig(
                ...
                extra_loss, exp18_diag = compute_propagation_gated_losses(
                    z_hat_x0,
                    prop_latents,
                    pos_latents,
                    masks,
                ...
                diagnostics.update(prop_conf_stats)
                diagnostics.update(exp18_diag)
                diagnostics["exp18_extra_loss"] = extra_loss.detach().item()
                diagnostics["total_loss"] = loss.detach().item()
                diagnostics["anchored_total_loss"] = loss.detach().item()
                diagnostics["propagation_target_mode"] = "latent_x0"
                ...
                if accelerator.sync_gradients:
                    ...
                    if args.preflight_only:
                        if accelerator.is_main_process:
                            ...
                            preflight_dir = Path("exp18_multiframe_propagation_gated_dpo") / "dpo_diag"
                            preflight_dir.mkdir(parents=True, exist_ok=True)
                            shutil.copyfile(
                                os.path.join(args.output_dir, "dpo_diagnostics.csv"),
                                preflight_dir / "preflight_dpo_diagnostics.csv",
                            )
                            logger.info("Exp18 preflight passed: wrote one DPO diagnostic row and skipped optimizer.step().")
