#!/usr/bin/env python3
"""Run the Exp35 MiniMax winner-SFT positive-control micro gate."""
...
def main() -> None:
    ...
    for lr in lrs:
        recipe = f"S0_adamw_lr{lr:g}".replace("-", "m")
        ...
        optimizer = torch.optim.AdamW(policy.parameters(), lr=lr, betas=(0.9, 0.95), eps=1e-8, weight_decay=1e-4)
        step_rows = []
        nan_detected = False
    ...
    write_json(reports_root / "exp35_minimax_winner_sft_summary.json", summary)
    write_csv(reports_root / "exp35_minimax_winner_sft_positive_control.csv", all_diag)
    write_csv(reports_root / "exp35_minimax_winner_sft_metrics.csv", all_metrics)
    write_csv(reports_root / "exp35_minimax_winner_sft_visual_review.csv", all_visual)
    md = [
    ...
    (reports_root / "exp35_minimax_winner_sft_positive_control.md").write_text("\n".join(md) + "\n", encoding="utf-8")
