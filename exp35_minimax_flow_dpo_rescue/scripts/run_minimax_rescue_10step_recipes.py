#!/usr/bin/env python3
"""Run preregistered Exp35 MiniMax 10-step rescue recipes."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--utility-scale", type=float, default=10.0)
...
def recipe_loss(policy: torch.nn.Module, reference: torch.nn.Module, cache, row: dict, recipe: str, scale: float, lambda_winner: float, lambda_outside: float) -> tuple[torch.Tensor, dict[str, float]]:
    ...
    outside = torch.zeros_like(loss)
    if recipe == "R3":
        anchor = winner_policy["weighted"] * lambda_winner
        outside = winner_policy["outside"] * lambda_outside
...
def main() -> None:
    ...
    if args.steps > 10:
        raise ValueError("Exp35 rescue recipe gate is capped at 10 steps")
    ...
    summary: dict[str, object] = {
        "status": "MINIMAX_RESCUE_10STEP_REQUIRES_CODEX_VISUAL_REVIEW",
    ...
    for recipe in recipes:
        ...
        zero_ok = (
            ...
            and abs(zero_diag["loss"] - math.log(2.0)) < 0.2
        )
        ...
        reference_delta = 0.0 if recipe != "R2" else delta_probe(reference, base_reload)
    ...
    write_json(reports_root / "exp35_minimax_rescue_10step_summary.json", summary)
    write_csv(reports_root / "exp35_minimax_rescue_10step_diagnostics.csv", all_diag_rows)
    write_csv(reports_root / "exp35_minimax_rescue_10step_metrics.csv", all_metric_rows)
    write_csv(reports_root / "exp35_minimax_rescue_10step_visual_review.csv", all_visual_rows)
    md = [
    ...
    (reports_root / "exp35_minimax_rescue_10step.md").write_text("\n".join(md) + "\n", encoding="utf-8")
