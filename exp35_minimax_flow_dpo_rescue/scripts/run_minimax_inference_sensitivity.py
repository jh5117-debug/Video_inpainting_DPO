#!/usr/bin/env python3
"""Exp35 MiniMax inference-sensitivity positive control.
...
Exp35 output root.  The goal is to prove whether MiniMax inference is sensitive
...
def main() -> None:
    ...
    for role, manifest_path, limit in (
        ...
        for row in read_jsonl(manifest_path)[:limit]:
            item = dict(row)
            item["exp35_sensitivity_role"] = role
            rows.append(item)
    if not rows:
    ...
    write_json(reports_root / "exp35_minimax_inference_sensitivity_perturb_tensors.json", touched)
    ...
    for idx, row in enumerate(rows):
        ...
        role = str(row["exp35_sensitivity_role"])
    ...
    write_csv(reports_root / "exp35_minimax_inference_sensitivity.csv", metric_rows)
    write_csv(reports_root / "exp35_minimax_inference_sensitivity_visual_review.csv", visual_rows)
    ...
    write_json(reports_root / "exp35_minimax_inference_sensitivity_summary.json", summary)
    md = [
    ...
    (reports_root / "exp35_minimax_inference_sensitivity.md").write_text("\n".join(md) + "\n", encoding="utf-8")
