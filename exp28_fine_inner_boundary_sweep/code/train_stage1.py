...
from exp23_two_stage_pool_morphology_sweep.code.pool_morphology import (
    ...
    build_pool_regions,
    build_region_weight_map as build_pool_region_weight_map,
    region_area_stats,
)
from exp28_fine_inner_boundary_sweep.code.inner_boundary_geometry import (
    ...
    region_area_stats as exp28_region_area_stats,
)
...
def build_exp23_loss_weight_map(
    brushnet_masks_image,
    latent_hw,
    args,
):
    """Build Exp23 pool-morphology region weights.

    `brushnet_masks_image` follows DiffuEraser convention: 0 = hole, 1 = known.
    Exp23 pool morphology uses a hole mask with 1 = hole.
    """
    if str(getattr(args, "exp28_geometry_mode", "") or "").strip():
        weight, stats, _ = build_exp28_loss_weight_map_and_regions(
            brushnet_masks_image,
            latent_hw,
            args,
        )
    ...
    if bool(getattr(args, "legacy_exact", False)):
    ...
    hole_latent = torch.nn.functional.interpolate(
        (1.0 - brushnet_masks_image.float()),
        size=(1, latent_hw[0], latent_hw[1]),
        mode="nearest",
    )
    ...
    weight_5d = build_pool_region_weight_map(
        hole_latent,
        config,
        core_weight=float(args.mask_region_weight),
        inner_weight=float(args.inner_weight),
        outer_weight=float(args.outer_weight),
        outside_weight=float(args.outside_region_weight),
    )
    regions = build_pool_regions(hole_latent, config)
    weight = rearrange(weight_5d.to(device=brushnet_masks_image.device, dtype=brushnet_masks_image.dtype), "b f c h w -> (b f) c h w")
    stats = region_area_stats(regions)
    stats.update(
        {
            ...
            "inner_weight": float(args.inner_weight),
            "outer_weight": float(args.outer_weight),
            "boundary_weight": float(args.outer_weight),
            "outside_weight": float(args.outside_region_weight),
            "region_mask_weight": float(args.mask_region_weight),
            "region_inner_weight": float(args.inner_weight),
            "region_outer_weight": float(args.outer_weight),
            "region_boundary_weight": float(args.outer_weight),
            "region_outside_weight": float(args.outside_region_weight),
            "pool_grid_scale": int(args.pool_grid_scale),
            "inner_pool_steps": int(args.inner_pool_steps),
            "outer_pool_steps": int(args.outer_pool_steps),
            "mask_area_ratio": float(regions["mask_core"].detach().float().mean().cpu()),
            "boundary_area_ratio": float((regions["inner_ring"] + regions["outer_ring"]).detach().float().mean().cpu()),
            "outside_area_ratio": float(regions["far_outside"].detach().float().mean().cpu()),
            "region_weight_sum": weight.sum(dim=[1, 2, 3]).mean().detach().item(),
        }
    )
    return weight, stats
...
def _exp28_inner_boundary_config(args):
    mode = str(getattr(args, "exp28_geometry_mode", "") or "").strip()
    if not mode:
        raise ValueError("missing exp28_geometry_mode")
    radius = int(getattr(args, "inner_radius_px", 0))
    if mode == "legacy_outer_one_ring":
        if radius != 0:
            raise ValueError("legacy_outer_one_ring requires inner_radius_px=0")
    elif mode == "inner_boundary_px":
        if radius <= 0:
            raise ValueError("inner_boundary_px requires inner_radius_px > 0")
    else:
        raise ValueError(f"unknown exp28_geometry_mode={mode!r}")
    return mode, InnerBoundaryConfig(
        inner_radius_px=radius,
        mask_core_weight=float(args.mask_region_weight),
        inner_weight=float(args.inner_weight),
        outer_weight=float(args.outer_weight),
        outside_weight=float(args.outside_region_weight),
    )
...
def build_exp28_loss_weight_map_and_regions(
    brushnet_masks_image,
    latent_hw,
    args,
):
    """Build Exp28 weight map and explicit regions.

    Input masks follow DiffuEraser convention: 0 = hole/object foreground,
    1 = known outside.  The loss-grid mask is derived by the same interpolation
    path as the Exp11/Exp23 fresh control; only the inner ring uses image-space
    erosion followed by area pooling.
    """
    mode, config = _exp28_inner_boundary_config(args)
    image_hole = (1.0 - brushnet_masks_image.float()).to(device=brushnet_masks_image.device)
    latent_known = torch.nn.functional.interpolate(
        brushnet_masks_image.to(dtype=torch.float32),
        size=(1, latent_hw[0], latent_hw[1]),
    )
    loss_hole = (1.0 - latent_known).to(device=brushnet_masks_image.device)
    regions = build_inner_boundary_regions(image_hole, loss_hole, config)
    weight_5d = build_inner_boundary_weight_map(image_hole, loss_hole, config)
    weight = rearrange(
        weight_5d.to(device=brushnet_masks_image.device, dtype=brushnet_masks_image.dtype),
        "b f c h w -> (b f) c h w",
    )
    stats = exp28_region_area_stats(regions)
    stats.update(
        {
            "boundary_mode": "exp28_inner_boundary",
            "exp23_spatial_boundary_mode": "exp28_inner_boundary",
            "exp28_geometry_mode": mode,
            "inner_radius_px": int(config.inner_radius_px),
            "mask_weight": float(config.mask_core_weight),
            "inner_weight": float(config.inner_weight),
            "outer_weight": float(config.outer_weight),
            "boundary_weight": float(config.outer_weight),
            "outside_weight": float(config.outside_weight),
            "region_mask_weight": float(config.mask_core_weight),
            "region_inner_weight": float(config.inner_weight),
            "region_outer_weight": float(config.outer_weight),
            "region_boundary_weight": float(config.outer_weight),
            "region_outside_weight": float(config.outside_weight),
            "pool_grid_scale": 1,
            "inner_pool_steps": 0,
            "outer_pool_steps": 1,
            "mask_area_ratio": float(regions["mask_core"].detach().float().mean().cpu()),
    ...
    region_maps = {
        "mask": rearrange(
            regions["mask_core"].to(device=brushnet_masks_image.device, dtype=brushnet_masks_image.dtype),
            "b f c h w -> (b f) c h w",
        ),
        "boundary": rearrange(
            (regions["inner_ring"] + regions["outer_ring"]).to(device=brushnet_masks_image.device, dtype=brushnet_masks_image.dtype),
            "b f c h w -> (b f) c h w",
        ),
        "outside": rearrange(
            regions["far_outside"].to(device=brushnet_masks_image.device, dtype=brushnet_masks_image.dtype),
            "b f c h w -> (b f) c h w",
        ),
    }
...
def build_exp23_loss_weight_map_and_regions(
    brushnet_masks_image,
    latent_hw,
    args,
):
    """Return Exp23 weight map plus explicit region maps for optional balancing."""
    if str(getattr(args, "exp28_geometry_mode", "") or "").strip():
        return build_exp28_loss_weight_map_and_regions(
            brushnet_masks_image,
            latent_hw,
            args,
        )

    weight, stats = build_exp23_loss_weight_map(
        brushnet_masks_image,
        latent_hw,
        args,
    )
    ...
    maps = build_pool_regions(hole_latent, config)
    region_maps = {
        "mask": rearrange(maps["mask_core"].to(device=brushnet_masks_image.device, dtype=brushnet_masks_image.dtype), "b f c h w -> (b f) c h w"),
        "boundary": rearrange((maps["inner_ring"] + maps["outer_ring"]).to(device=brushnet_masks_image.device, dtype=brushnet_masks_image.dtype), "b f c h w -> (b f) c h w"),
        "outside": rearrange(maps["far_outside"].to(device=brushnet_masks_image.device, dtype=brushnet_masks_image.dtype), "b f c h w -> (b f) c h w"),
    }
    return weight, stats, region_maps
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--inner_pool_steps", type=int, default=0,
                        help="Exp23 3x3 max-pool erosion steps used to carve the inner ring.")
    parser.add_argument("--outer_pool_steps", type=int, default=1,
                        help="Exp23 3x3 max-pool dilation steps used to form the outer ring.")
    parser.add_argument("--inner_weight", type=float, default=0.0,
                        help="Exp23 inner-ring region weight.")
    parser.add_argument("--outer_weight", type=float, default=None,
                        help="Exp23 outer-ring region weight. Defaults to --boundary_region_weight.")
    parser.add_argument("--exp28_geometry_mode", type=str, default="",
                        choices=["", "legacy_outer_one_ring", "inner_boundary_px"],
                        help="Exp28 explicit geometry. Empty keeps Exp23 compatibility.")
    parser.add_argument("--inner_radius_px", type=int, default=0,
                        help="Exp28 image-space inner erosion radius in pixels.")
    parser.add_argument("--radius_mode", type=str, default="legacy_latent_exact",
                        choices=["legacy_latent_exact"],
                        help="Compatibility alias retained for old trial configs; Exp23 uses pool-grid steps.")
    parser.add_argument("--radius_value", type=float, default=0.0,
                        help="Compatibility no-op retained for old trial configs.")
    ...
    parser.add_argument("--aggregation", type=str, default="legacy_global_weighted_mean",
                        ...
                        help="Exp23 loss aggregation: legacy global weighted mean or region-balanced region log-ratio.")
    ...
    parser.add_argument("--distance_cache_dir", type=str, default="",
                        help="Compatibility no-op. Exp23 pool morphology does not use image-space distance transforms.")
    ...
    if str(args.exp28_geometry_mode or ""):
        args.legacy_exact = False
        _exp28_inner_boundary_config(args)
    else:
        exp23_is_legacy = (
            int(args.pool_grid_scale) == 1
            and int(args.inner_pool_steps) == 0
            and int(args.outer_pool_steps) == 1
            and float(args.inner_weight) == 0.0
            and abs(float(args.outer_weight) - float(args.boundary_region_weight)) < 1e-12
        )
        if not exp23_is_legacy:
            args.legacy_exact = False
...
def write_exp20_run_metadata(args, accelerator):
    ...
    region_config = {
        ...
        "exp28_geometry_mode": str(getattr(args, "exp28_geometry_mode", "")),
        "inner_pool_steps": int(args.inner_pool_steps),
        "inner_radius_px": int(getattr(args, "inner_radius_px", 0)),
        "inner_weight": float(args.inner_weight),
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet_main, brushnet):
                ...
                if args.loss_region_mode == "region":
                    loss_weight_map, region_stats, loss_region_maps = build_exp23_loss_weight_map_and_regions(
