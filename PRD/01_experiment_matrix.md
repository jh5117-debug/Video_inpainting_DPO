...
| Exp31 strict base/protocol readback | `VIDEOPAINTER_2000_STRICT_READBACK_COMPLETE_BASE_AUDIT_PENDING` | Source/config readback supports the same official base family, locked search/shadow rows, 49F protocol, seed, mask polarity, first-frame-GT, and comp formula across Step0/50/2000. |
| Exp31 official base identity audit | `VIDEOPAINTER_BASE_IDENTITY_AUDIT_PASSED` | Official base and Step0 weights match exactly. Official/Step0/Step50/Step2000 replay on 2 search-dev + 2 shadow-dev rows matched existing raw/comp frames exactly (`MAE=0`, `max_abs=0`, 49/49 frames), and comp formula/polarity recomputation was exact. LPIPS/Ewarp remain required before formal positive. |
