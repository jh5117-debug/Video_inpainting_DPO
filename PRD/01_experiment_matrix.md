...
| `official_videodpo_diffueraser_data_partialmask_loser_comp_k4` | D2 ready; 5-step smoke passed; next formal run | DiffuEraser bridge | VideoDPO | VideoDPO winner | partial-mask inpainting + composite | partial K=4 | full | true | offline | data | PSNR, SSIM, VBench, SBS, DPO diagnostics | PAI output root: `/mnt/nas/hj/H20_Video_inpainting_DPO/data/generated_losers/official_videodpo_diffueraser_data_partialmask_loser_k4`; readiness passed with `ready=True`; `selected_primary_comp.repaired.jsonl`; Stage1 smoke 5/5, `dpo=0.6658`, `acc=1.0000`; `generation_source=diffueraser_only`, `stack=or`, `prior=propainter`. |
| `official_videodpo_diffueraser_data_partialmask_loser_nocomp_k4` | D2 ready; 5-step smoke passed | DiffuEraser bridge | VideoDPO | VideoDPO winner | partial-mask raw output | partial K=4 | full | false | offline | data diagnostic | PSNR, SSIM, VBench, SBS, DPO diagnostics | Reuses D2 raw loser manifest `selected_primary_nocomp.repaired.jsonl`; no new model inference; Stage1 smoke 5/5, `dpo=0.6521`, `acc=1.0000`. |
| `official_videodpo_diffueraser_task_partialmask_samemask_fullloss` | dataset entrypoint ready; 5-step smoke passed | DiffuEraser | generated partial-mask data | VideoDPO winner | partialmask comp loser | partial | same partial mask | true | offline data | task | metric.py, SBS, DPO diagnostics | Experiment 7. Uses `selected_primary_comp.repaired.jsonl` and `mask_path`; `M_train = M_gen`; loss remains full-video DPO loss; Stage1 smoke 5/5, `dpo=0.6533`, `acc=1.0000`. |
