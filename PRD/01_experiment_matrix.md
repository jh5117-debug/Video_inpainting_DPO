...
| --- | --- | --- |
| Exp59 VOID Kubric Gate8 official inference diagnostic | `EXP59_OFFICIAL_INFERENCE_PROTOCOL_READY` | `PRD/55_exp59_void_kubric_gate8_inference.md`, `experiment_registry/exp59_void_kubric_gate8_inference`, `reports/exp59_official_inference_protocol_audit.md` |
...
## Exp50 matrix update - VOID_WEIGHTS_READY
...
## Exp50 Gate8 matrix update - VOID_VOR_QUADMASK_GATE8_READY
...
| Exp51 | VOID loser-dominant rescue | Milestone D | `VOID_QUADMASK_ABLATION_READY` | Built Q0 current, Q1 object-only, Q2 strict affected, Q3 broad affected manifests for VOR-Train train4/heldout4. |
...
| Exp53B | VOID R1/R2 core recovery | Final | `EXP53B_ONESTEP_MIXED_ONLY` | R1_Q2_T500 ranked best H20 candidate, R2_Q2_T500 second. No one-step PASS; Exp55 aggregator should not run 10-step from Exp53B alone. |
| Exp55 | VOID cross-lane aggregator | Final | `EXP55_NO_10STEP_MIXED_ONLY` | Aggregated Exp53B H20 + Exp54 PAI one-step evidence. Best overall is H20 R1_Q2_T500_S0, best PAI is R4_Q2_T500_S0. All candidates are mixed-only under the original gate, so no 10-step is allowed. |
| Exp57 | VOID adaptive transition-safe DPO | `research/exp57-void-adaptive-transition-core-20260701` | `EXP57_READBACK_DONE` | VOID-only one-step repair for transition-region damage; no 10-step and no third-backbone claim. |
| Exp58 | VOID native data diagnostic | Milestone A | `EXP58_READBACK_DONE` | Official VOID data-generation audit completed; PAI/NAS preferred for large data. |
| Exp58 | VOID native data diagnostic | Milestone B | `VOID_KUBRIC_ENV_BLOCKED` | HAL wheelhouse relay succeeded and GCS manifests are reachable, but Kubric import requires TensorFlow and official VOID Kubric generation requires missing Blender/`bpy`. No Kubric data or 10-step. |
| Exp58 | VOID native data diagnostic | Final | `VOID_NATIVE_KUBRIC_BLOCKED` | Native-data mismatch test remains unexecuted because official Kubric generation is environment-blocked. No training, no inference on native data, no 10-step, no third-backbone claim. |
| Exp58B | VOID Kubric renderer env recovery | Milestone A | `EXP58B_READBACK_DONE` | Confirmed missing TensorFlow/Kubric import and missing Blender/`bpy`; PAI data/log/runtime writable but NAS tools/env roots require `/home/hj` fallback. |
| Exp58B | VOID Kubric renderer env recovery | Milestone B | `EXP58B_KUBRIC_PYTHON_ENV_READY` | Fresh PAI env imports TensorFlow/TFDS, official Google Research Kubric source with `AssetSource.from_manifest`, PyBullet, image stack, and OpenEXR. Blender/`bpy` remains pending. |
| Exp58B | VOID Kubric renderer env recovery | Milestone C | `EXP58B_BLENDER_BPY_READY` | Official Blender 3.6.23 runs headless from isolated tools fallback with user-extracted `libSM/libICE`; Blender Python imports `bpy`, TensorFlow, TFDS, source Kubric, PyBullet, and `kubric.renderer.Blender`. No render or training. |
| Exp58B | VOID Kubric renderer env recovery | Milestone D | `EXP58B_KUBRIC_INVOCATION_READY` | Added Exp58-only Blender Python launcher for the unmodified official script; local official manifests load, and `--num_pairs 0` dry-run completed with no rendered data. Gate1 next. |
| Exp58B | VOID Kubric renderer env recovery | Milestone E | `VOID_NATIVE_KUBRIC_GATE1_READY` | Generated one official Kubric pair at 128x128/24f; mask contains 0/63/127/255 and all evidence sheets opened. OpenEXR/ffmpeg compatibility fixed inside isolated Exp58B env. No inference/training. |
| Exp58B | VOID Kubric renderer env recovery | Milestone F/G | `VOID_DATA_MISMATCH_TEST_READY` | Generated Kubric Gate8, 8/8 decode and contain 0/63/127/255; all review pages opened. No VOID inference, training, one-step, or 10-step. Next: official VOID inference on Kubric Gate8. |
