...
| Exp60B VideoPainter VPData D3-mask scaleup | `EXP60B_READBACK_DONE` / `EXP60B_H20_READY_VIA_PAI_RELAY` / `EXP60B_VPDATA_SUBSET_PLAN_READY` / `EXP60B_H20_VPDATA_SUBSET_BLOCKED_NETWORK` | New VPData-subset branch created from Exp26. VPData is public; full clone/download is forbidden. PAI GPU0/GPU1 are ready. H20 is reachable through PAI relay and has 1.2T free; guarded Pexels-only train1000/test100 plan is ready, but H20/PAI outbound network cannot reach Hugging Face for raw download. |
