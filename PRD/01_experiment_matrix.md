...
| User-facing name | Status | Evidence / registry |
| --- | --- | --- |
| Exp45 PAI MiniMax pair scale-up | `MINIMAX_TARGETED_MINING_COMPLETED`; real PAI dsw mining produced 72 new candidates, visual relabel pending | `PRD/59_exp45_pai_minimax_pair_scaleup.md`, `experiment_registry/exp45_pai_minimax_pair_scaleup`, `reports/exp45_hal_environment_blocker_correction.md`, `reports/exp45_pair_scaleup_readback.md`, `reports/exp45_scope_deviation_h20_execution.md`, `reports/exp45_h20_handoff_package.md`, `reports/exp45_targeted_scaleup_mining.md`, `reports/exp45_visual_relabel.md`, `reports/exp45_stage2_formal_handoff.md` |
...
| Exp44 same-source pair construction | `MINIMAX_SAME_SOURCE_PAIR_GATE_PASSED` | Built 40 same-source pairs from visually relabeled pools, with train/search/shadow = 24/8/8, zero split scene-group overlap, GT background as DPO winner, pseudo-success retained only for Stage2 metadata, and same-source medium-hard MiniMax failures as losers. | Minimum pair gate passed; target 48 not reached. Unlocks bad-noise v4 construction only, not SFT/DPO training or MiniMax positive claims. |
| Exp44 bad-noise v4 states | `MINIMAX_BADNOISE_V4_READY` | Built 40 MiniMax-native bad-noise state records from same-source pairs. Usable H-states = 26 >= 24 gate; local/random gradient-proxy median ratio = 2.280567. | Unlocks Stage2-style dataset handoff preparation only. No training, optimizer step, VOR-Eval, hard comp, or MiniMax quality-positive claim. |
| Exp44 Stage2 dataset handoff | `MINIMAX_STAGE2_DATA_HANDOFF_PARTIAL` | Built GT distillation, pseudo-success distillation, and same-source preference manifests at train/search/shadow = 24/8/8 with zero scene overlap and 40/40 matched bad-noise states. | Marked `TRAINING_NOT_UNLOCKED` because counts are below 32/16/16 and current session could not validate `/mnt/nas` paths. H20 should start with pseudo-success SFT 30-step, not GT-only SFT. |
