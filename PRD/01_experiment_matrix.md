...
| Experiment | Status | Notes |
| --- | --- | --- |
| Exp31 VideoPainter 2000-step | `VIDEOPAINTER_2000_POSITIVE` | Step2000 improves over Step0 and Step50 on fixed search-dev and shadow-dev metrics, visual review, LPIPS, and mask-region Ewarp. Shadow comp Step2000 vs Step50: full PSNR `+2.305730`, full LPIPS `-0.008813`, mask LPIPS `-0.034082`, boundary PSNR `+3.637059`, Ewarp `-0.258536`, probability improved `>=0.9062`. |
| Exp31 strict base/protocol readback | `VIDEOPAINTER_2000_STRICT_READBACK_COMPLETE` | Source/config readback supports the same official base family, locked search/shadow rows, 49F protocol, seed, mask polarity, first-frame-GT, and comp formula across Step0/50/2000. Official replay and LPIPS/Ewarp completion passed. |
| Exp31 official base identity audit | `VIDEOPAINTER_BASE_IDENTITY_AUDIT_PASSED` | Official base and Step0 weights match exactly. Official/Step0/Step50/Step2000 replay on 2 search-dev + 2 shadow-dev rows matched existing raw/comp frames exactly (`MAE=0`, `max_abs=0`, 49/49 frames), and comp formula/polarity recomputation was exact. |
...
| Paper evidence after Exp31/Exp33 | `DIFFUERASER_PLUS_VIDEOPAINTER_POSITIVE_EFFECTERASE_BASELINE_ONLY` | `reports/exp31_vp_2000_paper_evidence.md` records DiffuEraser main evidence plus VideoPainter Step2000 long-run positive evidence. MiniMax is plumbing-only / not a third successful adapter, and EffectErase is weak baseline evidence only. |
