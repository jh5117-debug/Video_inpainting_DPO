...
| Exp43 H20 MiniMax Stage2 SFT runner readback | `H20_EXP43_STAGE2_SFT_RUNNER_READBACK_COMPLETED` | Exp41 blocker confirmed as runner-scope only: current MiniMax runners cap at 10 steps while the ladder needs true 30/100/300-step SFT. Exp43 may add isolated code only under `exp43_h20_minimax_stage2_sft_runner/`; H20 data/weights/protocol/BF16 prior gates were read back, Exp42 remains PAI-side data mining, and VOR-Eval remains excluded. |
...
| Exp43 / Exp44 pseudo-success H20 path validation | `H20_EXP43_EXP44_PSEUDOSUCCESS_PREFLIGHT_BLOCKED_MISSING_TARGETS` | H20 fetched Exp44 pseudo-success handoff manifests. Condition/mask assets resolve through H20 mirror for train/search/shadow `24/8/8`, but pseudo-success target frames and mp4s resolve `0/0/0`. | Do not launch pseudo-success 30-step yet; GT-only SFT remains forbidden as first experiment. Mirror Exp44 targeted mining outputs to H20, then revalidate paths. |

| Exp46 H20 MiniMax Exp45 Pseudo-Success SFT | MiniMax | pseudo-success Stage2 SFT validation | MINIMAX_PSEUDOSUCCESS_STAGE2_NEGATIVE | SFT30 negative; SFT100 not run; GT-only/DPO not run; MiniMax not third-backbone evidence. |
| Exp47 H20 MiniMax pseudo-success forensic | MiniMax | failure root-cause audit after Exp46 negative SFT30 | EXP47_FORENSIC_DECISION_COMPLETE | Root cause GLOBAL_SFT_SHOULD_BE_LOCALIZED; next step NEXT_H20_LOCAL_PSEUDOSUCCESS_TARGET_1_10STEP; no training/DPO/GT-only/100-step; MiniMax not third-backbone evidence. |
