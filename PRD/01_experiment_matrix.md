...
| Exp60B VideoPainter VPData D3-mask scaleup | `EXP60B_READBACK_DONE` / `EXP60B_H20_READY_VIA_PAI_RELAY` / `EXP60B_VPDATA_SUBSET_PLAN_READY` / `EXP60B_H20_VPDATA_SUBSET_BLOCKED_PROXY` / `EXP60B_HAL_VPDATA_SUBSET_BLOCKED` / `EXP60B_TRANSFER_BLOCKED` | H20 hf-mirror unblocked metadata and downloaded 1,089/1,100 locked Pexels raw videos; H20 clash proxy remained at 1,089/1,100; HAL missing-URL probe also returned HTTP 403 for all 11 missing locked URLs. PAI ready transfer and D3 mask generation remain blocked. No full VPData download, GPU job, loser generation, inference, or training was run. |
| Exp60C VideoPainter VPData failed-URL replacement | `EXP60C_H20_VPDATA_SUBSET_READY` / `EXP60C_TRANSFER_BLOCKED` | Deterministic same-split Pexels-only replacements resolved the 11 HTTP-403 rows. H20 now has train1000/test100, 1,100 sha256 rows, and 1,100/1,100 OpenCV decode pass after targeted repair of 6 incomplete MP4s. PAI/NAS transfer is blocked because `/mnt/nas/hj/H20_Video_inpainting_DPO/data/external` is not writable by `hj`. No masks, losers, inference, DPO, training, GPU use, or full VPData download ran. |
| Exp60C PAI target permission recovery | `EXP60C_PAI_TARGET_PERMISSION_RECOVERED` | Root-side permission fix was already completed. Codex verified all Exp60C PAI/NAS target dirs are `hj:hj`, mode `0770`, and pass read/write/execute/write_probe. Next allowed milestone is transfer and verification only. |
| Exp60C VPData subset transfer to PAI/NAS | `EXP60C_PAI_VPDATA_SUBSET_READY` | Transferred the repaired train1000/test100 raw subset from H20 to PAI/NAS. PAI has 1,100 MP4s, 14G by `du`, 1,100/1,100 SHA256 match, 1,100/1,100 OpenCV decode pass, no train/test overlap, no duplicate source IDs/URLs/paths, and PAI manifests with no H20/HAL path leakage. No masks, losers, inference, DPO, training, GPU use, or full VPData download ran. |
| Exp60C final download decision | `VIDEOPAINTER_VPDATA_SUBSET_READY_ON_PAI` | Raw VPData train1000/test100 subset is ready for the next separately gated PAI D3 mask generation milestone. This is not VPData validation, not DPO positive, and not a training result. |
