...
training. Validate the BR/no-prior replacement before any full D1 training.
...
used for training. Current H20-2 D1 artifacts are suspect because they were
generated by the OR path under a full-frame mask. This may make the loser
trivially bad rather than a useful hard negative.
...
New H20-2 BR/no-prior validation root:
...
- New BR/no-prior `_shards/`: per-pair shard outputs and logs while validation is running.
- New BR/no-prior `manifests/`: merged manifests after the launcher reaches the end of the requested range.
- New BR/no-prior `reports/`: merged generation report after the launcher reaches the end of the requested range.

Current D1 BR/no-prior validation command:
...
The 20-sample D1 gate has passed. Full D1 generation is now running on H20-2 as
documented above.
