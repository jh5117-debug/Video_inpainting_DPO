...
def compute_dpo_loss(
    ...
    adaptive_norm_mode = os.environ.get("ADAPTIVE_NORM_MODE", "none").strip().lower()
    adaptive_norm_eps = float(os.environ.get("ADAPTIVE_NORM_EPS", "1e-6"))
    adaptive_win_gap_vec = None
    adaptive_lose_gap_vec = None
    if adaptive_norm_mode in {"", "none", "null"}:
        adaptive_norm_mode = "none"
    elif adaptive_norm_mode == "batch_zscore":
        win_std = dpo_win_gap_vec.detach().float().std(unbiased=False).clamp(min=adaptive_norm_eps)
        lose_std = dpo_lose_gap_vec.detach().float().std(unbiased=False).clamp(min=adaptive_norm_eps)
        adaptive_win_gap_vec = (dpo_win_gap_vec - dpo_win_gap_vec.detach().float().mean()) / win_std
        adaptive_lose_gap_vec = (dpo_lose_gap_vec - dpo_lose_gap_vec.detach().float().mean()) / lose_std
        dpo_win_gap_vec = adaptive_win_gap_vec
        dpo_lose_gap_vec = adaptive_lose_gap_vec
    else:
        raise ValueError(f"Unsupported ADAPTIVE_NORM_MODE={adaptive_norm_mode!r}")
    ...
    diagnostics = {
        ...
        "adaptive_norm_mode": adaptive_norm_mode,
        "adaptive_norm_win_gap": (
            adaptive_win_gap_vec.mean().detach().item() if adaptive_win_gap_vec is not None else "null"
        ),
        "adaptive_norm_lose_gap": (
            adaptive_lose_gap_vec.mean().detach().item() if adaptive_lose_gap_vec is not None else "null"
        ),
        "adaptive_norm_lose_gap_clipped": (
            dpo_lose_gap_clipped_vec.mean().detach().item() if adaptive_lose_gap_vec is not None else "null"
        ),
...
def build_region_loss_weight_map(
    ...
    stats = {
        "boundary_mode": os.environ.get("BOUNDARY_MODE", "exp10_default").strip().lower() or "exp10_default",
