...
def summarize_diag(diags: list[dict[str, Any]]) -> dict[str, float]:
    ...
    ratios = [
        abs(float(d.get("effective_loser_gap", 0.0))) / max(abs(float(d["preference_margin"])), 1e-12)
        for d in diags
        if "preference_margin" in d
    ]
...
def main() -> None:
    ...
    out_root = Path(args.output_root) / "core_forward" / args.cell
    ...
    log(f"{args.cell}: load policy transformer gradient_checkpointing={args.gradient_checkpointing}")
    ...
    log(f"{args.cell}: load cached train4")
    ...
    for item in train_items:
        ...
        log(f"{args.cell}: train sample {item['sample_id']} loss={scalar(loss):.8f}")
    ...
    log(f"{args.cell}: strict reload transformer")
    ...
    heldout_finite = True
    log(f"{args.cell}: heldout4 finite forward")
    with torch.no_grad():
    ...
    log(json.dumps({"cell": args.cell, "status": status, "checkpoint": str(ckpt), "runtime_sec": runtime}, sort_keys=True))
