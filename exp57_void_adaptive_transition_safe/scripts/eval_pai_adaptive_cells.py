...
def main() -> None:
    ...
    with (REPORTS / "exp57_pai_adaptive_onestep_metrics.csv").open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=metric_fields)
        writer.writeheader()
        writer.writerows(all_metrics)
    review_fields = sorted({k for row in all_reviews for k in row})
    with (REPORTS / "exp57_pai_adaptive_onestep_visual_review.csv").open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=review_fields)
        writer.writeheader()
        writer.writerows(all_reviews)
    diag_fields = sorted({k for row in diag_rows for k in row})
    with (REPORTS / "exp57_pai_adaptive_onestep_diagnostics.csv").open("w", newline="") as f:
    ...
    overall = "EXP57_PAI_ADAPTIVE_ONESTEP_PASS" if any(s["status"] == "PASS" for s in summary.values()) else "EXP57_PAI_ADAPTIVE_ONESTEP_MIXED"
    out = {
        "status": overall,
        "best_cell": best,
        "cells": summary,
    ...
    (REPORTS / "exp57_pai_adaptive_onestep_summary.json").write_text(json.dumps(out, indent=2, sort_keys=True) + "\n")
    lines = [
        "# Exp57 PAI Adaptive Transition One-Step",
        "",
        f"Status: `{overall}`",
        "",
    ...
    lines.extend([
        "",
        f"Best PAI cell: `{best}`.",
        "",
        "No 10-step was run. No VOID third-backbone evidence is claimed.",
    ])
    (REPORTS / "exp57_pai_adaptive_onestep.md").write_text("\n".join(lines) + "\n")
