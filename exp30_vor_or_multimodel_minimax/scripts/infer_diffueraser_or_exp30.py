#!/usr/bin/env python3
"""Exp30 DiffuEraser OR wrapper with explicit PCM identity.
...
"""
    no_pcm_block = """        ## Exp30 explicit no-PCM mode.
        ## This is a separate generator identity, not a fallback.
        self.ckpt = "none"
        self.guidance_scale = float(os.environ.get("EXP30_NO_PCM_GUIDANCE", "0.0"))
        self.num_inference_steps = int(os.environ.get("EXP30_NO_PCM_STEPS", "6"))
        loaded = "none"
"""
    ...
    for child in (project_root / "inference").iterdir():
        ...
        if child.name == "run_OR.py":
            ...
            if "parser.add_argument(\"--seed\"" not in text:
                ...
                text = text.replace(anchor, anchor + '\n    parser.add_argument("--seed", type=int, default=None, help="Exp30 overlay seed for DiffuEraser noise")')
...
def main() -> None:
    ...
    if args.pcm_mode == "none":
        env["EXP30_NO_PCM_STEPS"] = str(args.no_pcm_steps)
        env["EXP30_NO_PCM_GUIDANCE"] = str(args.no_pcm_guidance)
    ...
    print("[exp30-diffueraser] run:", " ".join(cmd), flush=True)
    identity = {
        "wrapper": "exp30_vor_or_multimodel_minimax/scripts/infer_diffueraser_or_exp30.py",
    ...
    print("[exp30-diffueraser] identity:", identity, flush=True)
    ...
    print(f"[exp30-diffueraser] saved {saved} frames to {output_dir}")
