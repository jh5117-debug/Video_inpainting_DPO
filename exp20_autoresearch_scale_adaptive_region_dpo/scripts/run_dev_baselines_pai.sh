...
RUN_TAG="${RUN_TAG:-$(date +%Y%m%d_%H%M%S)_dev_boundary_search_v1_baselines}"
...
DEV_ROOT="${DEV_ROOT:-${EXP_ROOT}/logs/target_eval/exp20_autoresearch_scale_adaptive_region_dpo/dev_boundary_search_v1_e385cc27_roots}"
...
if [[ "${video_count}" -ne 16 || "${mask_count}" -ne 16 ]]; then
  echo "[bad-dev-root] expected 16 video dirs and 16 mask dirs, got video=${video_count} mask=${mask_count}" >&2
  exit 3
fi
...
labels=(SFT48000_baseline Exp11_outer_b075_S1_plus_SFT_S2 Exp11_outer_b075_S2 Exp20_legacy_exact_untrained_control)
weights=("${SFT48000}" "${EXP11_S1_HYBRID}" "${EXP11_S2}" "${SFT48000}")
...
out = reports / "exp20_dev_baselines.csv"
keys = []
for row in rows:
    for key in row:
        if key not in keys:
...
target = max(sft + 0.35, s1 + 0.05)
summary = {
    "SFT_DEV_PSNR": sft,
    "EXP11_S1_DEV_PSNR": s1,
    "EXP11_S2_DEV_PSNR": s2,
    "TARGET_DEV_PSNR": target,
}
(reports / "exp20_dev_baseline_targets.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
...
lines = [
    ...
    f"- SFT_DEV_PSNR: `{sft:.6f}`",
    f"- EXP11_S1_DEV_PSNR: `{s1:.6f}`",
    f"- EXP11_S2_DEV_PSNR: `{s2:.6f}`",
    f"- TARGET_DEV_PSNR: `{target:.6f}`",
    "",
...
for row in rows:
    ...
    lines.append("| {} | {} | {} |".format(row["model_label"], " | ".join(values), row.get("rows", "")))
(reports / "exp20_dev_baselines.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
print("\n".join(lines))
PY

cat > "${OUT_ROOT}/report.md" <<EOF
# Exp20 Dev Boundary Search v1 Baselines

- dev_root: \`${DEV_ROOT}\`
- output_root: \`${OUT_ROOT}\`
- logs: \`${LOG_ROOT}\`
- summary: \`${PROJECT_ROOT}/reports/exp20_dev_baselines.csv\`
EOF
