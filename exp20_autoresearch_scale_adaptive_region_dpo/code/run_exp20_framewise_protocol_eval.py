#!/usr/bin/env python3
"""Run the locked Exp20 frame-wise DiffuEraser evaluation protocol.

This is an Exp20-local copy of the frame-wise evaluator. The only protocol
change is that the DiffuEraser inference seed is explicit and fixed by default
so SFT and legacy control runs are directly comparable.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--inference_seed", type=int, default=20260619)
...
def main() -> int:
    ...
    report = [
        "# Exp20 Frame-wise Protocol Summary",
        "",
        f"label: `{args.label}`",
        f"rows: {len(rows)}",
        f"protocol: raw6, no PCM, mask dilation 0, no Gaussian blur, hard comp, frame-wise metric, inference_seed {args.inference_seed}",
