...
GPU="$1"
PORT="$2"
BASE="exp20_autoresearch_scale_adaptive_region_dpo/adaptive_search"
LOGROOT="/mnt/nas/hj/H20_Video_inpainting_DPO/logs/autoresearch/exp20/adaptive_search"
mkdir -p "$BASE/running" "$BASE/done" "$BASE/crash" "$LOGROOT"
LOCK="/tmp/exp20_adaptive_gpu_${GPU}.flock"
exec 9>"$LOCK"
if ! flock -n 9; then
