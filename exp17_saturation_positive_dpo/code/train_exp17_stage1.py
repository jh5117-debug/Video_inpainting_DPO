...
def compute_dpo_loss(
    ...
    dpo_loss_raw_vec = -1.0 * F.logsigmoid(frame_inside_term)

    exp17_variant = os.environ.get("EXP17_VARIANT", "none").strip().lower()
    if exp17_variant in {"", "off", "false", "0"}:
        exp17_variant = "none"
    valid_exp17_variants = {"none", "positive", "dpop", "saturation", "combined"}
    if exp17_variant not in valid_exp17_variants:
        raise ValueError(f"Unsupported EXP17_VARIANT={exp17_variant!r}; expected one of {sorted(valid_exp17_variants)}")

    exp17_lambda_abs = float(os.environ.get("EXP17_LAMBDA_ABS", winner_abs_reg_weight))
    exp17_lambda_pos = float(os.environ.get("EXP17_LAMBDA_POS", winner_gap_reg_weight))
    exp17_margin_pos = float(os.environ.get("EXP17_MARGIN_POS", winner_gap_reg_margin))
    exp17_target_margin = float(os.environ.get("EXP17_TARGET_MARGIN", "1.0"))
    exp17_saturation_kappa = float(os.environ.get("EXP17_SATURATION_KAPPA", "5.0"))

    margin_pref_vec = float(lose_gap_weight) * dpo_lose_gap_clipped_vec - dpo_win_gap_vec
    sat_weight_vec = torch.sigmoid(exp17_saturation_kappa * (exp17_target_margin - margin_pref_vec))
    dpo_loss_saturated = (dpo_loss_raw_vec * sat_weight_vec.detach()).mean()

    # 训练 loss 保持原有 frame-level 目标；诊断统一按 video-pair 统计。
    dpo_loss_raw = dpo_loss_raw_vec.mean()
    dpo_loss = dpo_loss_saturated if exp17_variant in {"saturation", "combined"} else dpo_loss_raw
    ...
    active_margin_pos = exp17_margin_pos if exp17_variant in {"positive", "dpop", "combined"} else float(winner_gap_reg_margin)
    relu_win_gap_vec = F.relu(win_gap_vec - active_margin_pos)
    winner_gap_reg = relu_win_gap_vec.mean()

    if exp17_variant in {"positive", "dpop", "saturation", "combined"}:
        lambda_abs_active = exp17_lambda_abs
        if exp17_variant in {"saturation"}:
            lambda_pos_active = float(winner_gap_reg_weight)
        else:
            lambda_pos_active = exp17_lambda_pos
        loss = dpo_loss + lambda_abs_active * winner_abs_reg + lambda_pos_active * winner_gap_reg
    else:
        lambda_abs_active = float(winner_abs_reg_weight)
        lambda_pos_active = float(winner_gap_reg_weight)
        loss = (
            dpo_loss
            + float(sft_reg_weight) * sft_loss
            + lambda_abs_active * winner_abs_reg
            + lambda_pos_active * winner_gap_reg
        )
    ...
    pair_margin_pref = float(lose_gap_weight) * pair_dpo_lose_gap_clipped - pair_dpo_win_gap
    pair_sat_weight = torch.sigmoid(exp17_saturation_kappa * (exp17_target_margin - pair_margin_pref))
    saturated_pair_ratio = (pair_margin_pref >= exp17_target_margin).float().mean()
    ...
    diagnostics = {
        ...
        "exp17_variant": exp17_variant,
        "dpo_loss_raw": dpo_loss_raw.detach().item(),
        "dpo_loss_saturated": dpo_loss_saturated.detach().item(),
        ...
        "winner_abs_reg_weight": float(lambda_abs_active),
        "winner_gap_reg": winner_gap_reg.detach().item(),
        "winner_gap_reg_weight": float(lambda_pos_active),
        "winner_gap_reg_margin": float(active_margin_pos),
        "winner_gap_reg_mode": winner_gap_reg_mode,
        "lambda_abs": float(lambda_abs_active),
        "lambda_pos": float(lambda_pos_active),
        "margin_pos": float(active_margin_pos),
        "margin_pref": pair_margin_pref.detach().mean().item(),
        "target_margin": float(exp17_target_margin),
        "sat_weight_mean": pair_sat_weight.detach().mean().item(),
        "saturated_pair_ratio": saturated_pair_ratio.detach().item(),
        "saturation_kappa": float(exp17_saturation_kappa),
...
def append_dpo_diagnostics_csv(output_dir, step, diag, grad_norm=None):
    ...
    row = {
        "global_step": step,
        "exp17_variant": diag.get("exp17_variant"),
        "dpo_loss": diag.get("dpo_loss"),
        "dpo_loss_raw": diag.get("dpo_loss_raw"),
        "dpo_loss_saturated": diag.get("dpo_loss_saturated"),
        ...
        "lambda_abs": diag.get("lambda_abs"),
        "lambda_pos": diag.get("lambda_pos"),
        "margin_pos": diag.get("margin_pos"),
        "margin_pref": diag.get("margin_pref"),
        "target_margin": diag.get("target_margin"),
        "sat_weight_mean": diag.get("sat_weight_mean"),
        "saturated_pair_ratio": diag.get("saturated_pair_ratio"),
        "saturation_kappa": diag.get("saturation_kappa"),
