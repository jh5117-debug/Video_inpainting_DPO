#!/usr/bin/env python3
"""Build Exp17 DAVIS visual sanity panels from frame-wise eval outputs."""
...
def build_video(
    ...
    exp17 = load_frames(exp17_root / video / "diffueraser_comp_frames", video_length)
    count = min(len(gt), len(masks), len(sft), len(exp11), len(exp17), video_length)
    if count == 0:
        raise RuntimeError(f"empty visual case for {video}")
    gt, masks, sft, exp11, exp17 = gt[:count], masks[:count], sft[:count], exp11[:count], exp17[:count]
    ...
    for idx, (gt_frame, mask_frame, sft_frame, exp11_frame, exp17_frame) in enumerate(zip(gt, masks, sft, exp11, exp17)):
        sft_frame = resize_like(sft_frame, gt_frame)
        exp11_frame = resize_like(exp11_frame, gt_frame)
        exp17_frame = resize_like(exp17_frame, gt_frame)
        panel = np.concatenate(
            [
                ...
                add_label(exp17_frame, exp17_label),
            ],
            axis=1,
        )
        panels.append(panel)
    ...
    exp17_metrics = read_metrics(exp17_root).get(video, {})
    return {
        "video": video,
        "frames": str(count),
        ...
        "exp17_psnr": exp17_metrics.get("whole_video_psnr", ""),
        "exp17_minus_exp11_psnr": metric_delta(exp17_metrics, exp11_metrics, "whole_video_psnr"),
        "exp17_minus_sft_psnr": metric_delta(exp17_metrics, sft_metrics, "whole_video_psnr"),
        "sft_ssim": sft_metrics.get("whole_video_ssim", ""),
        "exp11_ssim": exp11_metrics.get("whole_video_ssim", ""),
        "exp17_ssim": exp17_metrics.get("whole_video_ssim", ""),
        "exp17_minus_exp11_ssim": metric_delta(exp17_metrics, exp11_metrics, "whole_video_ssim"),
        "exp17_strict_mask_psnr": exp17_metrics.get("strict_mask_pixel_psnr", ""),
        "exp11_strict_mask_psnr": exp11_metrics.get("strict_mask_pixel_psnr", ""),
        "exp17_minus_exp11_strict_mask_psnr": metric_delta(exp17_metrics, exp11_metrics, "strict_mask_pixel_psnr"),
    }
...
def main() -> int:
    ...
    parser.add_argument("--exp17_eval_root", required=True)
    parser.add_argument("--exp17_label", default="Exp17")
    ...
    lines = [
        "# Exp17 DAVIS Visual Sanity",
        "",
        "| Video | SFT PSNR | Exp11 PSNR | Exp17 PSNR | Exp17-Exp11 PSNR | Exp17-Exp11 SSIM | Contact sheet |",
        "|---|---:|---:|---:|---:|---:|---|",
    ]
    for row in rows:
        lines.append(
            "| {video} | {sft_psnr} | {exp11_psnr} | {exp17_psnr} | {exp17_minus_exp11_psnr} | "
            "{exp17_minus_exp11_ssim} | {contact_sheet} |".format(**row)
        )
    (output_root / "report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"[exp17-visual] wrote {manifest}")
    return 0
