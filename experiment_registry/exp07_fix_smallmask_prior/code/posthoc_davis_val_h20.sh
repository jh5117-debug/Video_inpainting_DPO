...
PROPAINTER_WEIGHT_ROOT="${PROPAINTER_WEIGHT_ROOT:-${WEIGHTS_DIR}/propainter}"
RAFT_MODEL_PATH="${RAFT_MODEL_PATH:-}"
EXP8C_DAVIS_HELPER="${EXP8C_DAVIS_HELPER:-${PROJECT_ROOT}/experiment_registry/exp08c_youtubevos_gtwin_d3comp_fullloss_davis_s1s2_2000/code/launch_s1s2_pai.sh}"
...
require_path "${EXP8C_DAVIS_HELPER}" "Exp8c DAVIS helper source"
...
awk '/^run_stage1\(\)/ {exit} {print}' \
  "${EXP8C_DAVIS_HELPER}" > "${helper_file}"
