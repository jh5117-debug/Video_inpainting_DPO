...
VIDEO_BASE = OUT_BASE / "wave1_video"
...
CELLS = ["R3_Q2_T500_S0", "R4_Q2_T500_S0"]
...
def main() -> None:
    ...
    with (REPORTS / "exp54_sdpo_linear_onestep_metrics.csv").open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        writer.writerows(metric_rows)
    with (REPORTS / "exp54_sdpo_linear_onestep_visual_review.csv").open("w", newline="") as f:
    ...
    for cell in CELLS:
        ...
        with diag_path.open(newline="") as f:
            for d in csv.DictReader(f):
                ...
                diag_rows.append(d)
    with (REPORTS / "exp54_sdpo_linear_onestep_diagnostics.csv").open("w", newline="") as f:
    ...
    status = "EXP54_SDPO_LINEAR_ONESTEP_PASS" if any_pass else ("EXP54_SDPO_LINEAR_ONESTEP_MIXED" if any_mixed else "EXP54_SDPO_LINEAR_ONESTEP_NEGATIVE")
    summary["status"] = status
    (REPORTS / "exp54_sdpo_linear_onestep_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    ...
    (REPORTS / "exp54_sdpo_linear_onestep.md").write_text("\n".join(md) + "\n")
    print(status)
