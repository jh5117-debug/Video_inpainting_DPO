...
CODE_ROOT="${EXP19_CODE_ROOT:-/mnt/workspace/hj/nas_hj/H20_Video_inpainting_DPO_exp09_10_11_pai_sync}"
if [[ ! -d "${ROOT}" ]]; then
  ROOT="/mnt/nas/hj/H20_Video_inpainting_DPO"
fi
cd "${ROOT}"

export EXP19_CODE_ROOT="${CODE_ROOT}"
export PYTHONPATH="${ROOT}:${CODE_ROOT}:${PYTHONPATH:-}"
...
EXP19_ADAPTER="${EXP19_ADAPTER:-${OUTPUT_ROOT}/experiments/dpo/stage2/exp19b_boundary_flow_adapter_s2_500_limit100/last_weights/flow_adapter.pt}"
FLOW_CACHE="${FLOW_CACHE:-${OUTPUT_ROOT}/data/cache/exp19_davis10_propainter_completed_flow}"
OUT="${OUT:-${OUTPUT_ROOT}/logs/target_eval/exp19_boundary_gated_flow_adapter_davis10}"

INPUT_SIZE="${INPUT_SIZE:-432x240}"
VIDEO_LENGTH="${VIDEO_LENGTH:-24}"
NFRAMES="${NFRAMES:-22}"
NUM_INFERENCE_STEPS="${NUM_INFERENCE_STEPS:-6}"
LIMIT_VIDEOS="${LIMIT_VIDEOS:-10}"
SEED="${SEED:-1234}"
DEVICE="${DEVICE:-cuda}"
...
[[ "${COMPUTE_TC:-1}" == "1" ]] && args+=(--compute_tc --tc_model_path "${TC_MODEL_PATH:-${WEIGHTS_DIR}/open_clip_vit_h14}")
...
[[ "${EXP19_PREFLIGHT_ONLY:-0}" == "1" ]] && args+=(--preflight_only)
[[ "${EXP19_SKIP_PREFLIGHT:-0}" == "1" ]] && args+=(--skip_preflight)
