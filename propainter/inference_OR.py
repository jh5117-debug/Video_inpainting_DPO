...
#  video_length: max number of frames; <=0 (e.g. -1) = all frames (align with run_BR)
def read_frame_from_videos(frame_root, video_length):
    """Load frames from a video file or frame directory.
    ...
    if frame_root.endswith(('mp4', 'mov', 'avi', 'MP4', 'MOV', 'AVI')):  # input video path
        ...
        frames = [Image.fromarray(f) for f in list(vframes.numpy())]
        ...
        # Truncate by frames early for speed
        try:
            vl = int(video_length) if video_length is not None else -1
        except Exception:
            vl = -1
...
class Propainter:
    ...
    def forward(self, video, mask, output_path, resize_ratio=0.6, video_length=2, height=-1, width=-1,
                ...
                raft_iter=20, save_fps=24, save_frames=False, fp16=True, return_frames=False):
        """
        Args:
            return_frames: If True, return list of RGB numpy arrays instead of just saving video.
                          This avoids video compression loss for metrics computation.
        """
        ...
        print(f'Priori generating: [{video_length} frames]...', flush=True)
        with torch.no_grad():
            # ---- compute flow ----
            ...
            if frames.size(1) > short_clip_len:
                ...
                flow_range = list(range(0, video_length, short_clip_len))
                for f in tqdm(flow_range, desc='RAFT flow', leave=True):
            ...
            # === Memory optimization: delete flow lists after concatenation ===
            if 'gt_flows_f_list' in dir():
            ...
            if(len(frames[0]))>subvideo_length_img_prop: # perform propagation only when length of frames is larger than subvideo_length_img_prop
                ...
                if sample_frames.size(1) > short_clip_len:
                    gt_flows_f_list, gt_flows_b_list = [], []
                    sample_flow_range = list(range(0, sample_video_length, short_clip_len))
                    for f in tqdm(sample_flow_range, desc='RAFT flow (sample)', leave=True):
        ...
        # === Memory optimization: delete gt_flows_bi before transformer (pred_flows_bi still needed) ===
        if 'gt_flows_bi' in dir():
            del gt_flows_bi
        torch.cuda.empty_cache()
        gc.collect()
        ...
        for f in tqdm(range(0, video_length, neighbor_stride)):
            ...
            # === Memory optimization: explicit cleanup after each iteration ===
        ...
        if return_frames:
            return comp_frames  # List of RGB numpy arrays
        else:
            return output_path
